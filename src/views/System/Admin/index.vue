            ...
            <template #default="scope">
              <el-tooltip :content="scope.row.status ? '启用' : '停用'" placement="top">
                <el-switch v-model="scope.row.status" class="ml-2" inline-prompt :active-value="1" :inactive-value="0"
                           style="--el-switch-on-color: #13ce66; --el-switch-off-color: #ff4949" :disabled="scope.row.isAdmin">
                </el-switch>
              </el-tooltip>
            </template>
          </el-table-column>
...
import {AdminPageReturnType, AdminPageType} from "@/types/ums/admin"
...
// 搜索条件
const searchOptions: UnwrapNestedRefs<Partial<AdminPageType>> = reactive<Partial<AdminPageType>>({
  username: '',
  phone: '',
  status: -1,
  pageNo: PAGE_NO, // 页码
...
// 分页查询数据
const tableData = ref<AdminPageReturnType[]>([])

// 分页查询
const paginationQuery = async () => {
  const result: Result<PaginationReturn<AdminPageReturnType>> = await adminPageListApi(searchOptions)
